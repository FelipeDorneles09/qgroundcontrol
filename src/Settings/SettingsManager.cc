...
SettingsManager::SettingsManager(QObject* parent) : QObject(parent) { qCDebug(SettingsManagerLog) << this; }

SettingsManager::~SettingsManager() { qCDebug(SettingsManagerLog) << this; }
...
void SettingsManager::registerQmlTypes() {
    ...
    (void)qmlRegisterUncreatableType<SettingsManager>("QGroundControl.SettingsManager", 1, 0, "SettingsManager",
                                                      kRefOnly);

    /* App / global settings */
    (void)qmlRegisterUncreatableType<AppSettings>("QGroundControl.SettingsManager", 1, 0, "AppSettings", kRefOnly);
    (void)qmlRegisterUncreatableType<UnitsSettings>("QGroundControl.SettingsManager", 1, 0, "UnitsSettings", kRefOnly);
    (void)qmlRegisterUncreatableType<AutoConnectSettings>("QGroundControl.SettingsManager", 1, 0, "AutoConnectSettings",
                                                          kRefOnly);
    (void)qmlRegisterUncreatableType<BrandImageSettings>("QGroundControl.SettingsManager", 1, 0, "BrandImageSettings",
                                                         kRefOnly);

    /* Flight / map / plan settings */
    (void)qmlRegisterUncreatableType<FlightMapSettings>("QGroundControl.SettingsManager", 1, 0, "FlightMapSettings",
                                                        kRefOnly);
    (void)qmlRegisterUncreatableType<PlanViewSettings>("QGroundControl.SettingsManager", 1, 0, "PlanViewSettings",
                                                       kRefOnly);
    (void)qmlRegisterUncreatableType<MapsSettings>("QGroundControl.SettingsManager", 1, 0, "MapsSettings", kRefOnly);
    (void)qmlRegisterUncreatableType<OfflineMapsSettings>("QGroundControl.SettingsManager", 1, 0, "OfflineMapsSettings",
                                                          kRefOnly);

    /* Video / camera settings */
    (void)qmlRegisterUncreatableType<VideoSettings>("QGroundControl.SettingsManager", 1, 0, "VideoSettings", kRefOnly);

    /* Vehicle / firmware related settings */
    (void)qmlRegisterUncreatableType<FirmwareUpgradeSettings>("QGroundControl.SettingsManager", 1, 0,
                                                              "FirmwareUpgradeSettings", kRefOnly);
    (void)qmlRegisterUncreatableType<MavlinkSettings>("QGroundControl.SettingsManager", 1, 0, "MavlinkSettings",
                                                      kRefOnly);
#ifndef QGC_NO_ARDUPILOT_DIALECT
    (void)qmlRegisterUncreatableType<APMMavlinkStreamRateSettings>("QGroundControl.SettingsManager", 1, 0,
                                                                   "APMMavlinkStreamRateSettings", kRefOnly);
#endif

    /* UI / view settings */
    (void)qmlRegisterUncreatableType<FlyViewSettings>("QGroundControl.SettingsManager", 1, 0, "FlyViewSettings",
                                                      kRefOnly);
    (void)qmlRegisterUncreatableType<FlightModeSettings>("QGroundControl.SettingsManager", 1, 0, "FlightModeSettings",
                                                         kRefOnly);
    (void)qmlRegisterUncreatableType<RemoteIDSettings>("QGroundControl.SettingsManager", 1, 0, "RemoteIDSettings",
                                                       kRefOnly);

    /* RTK / telemetry */
    (void)qmlRegisterUncreatableType<RTKSettings>("QGroundControl.SettingsManager", 1, 0, "RTKSettings", kRefOnly);
    (void)qmlRegisterUncreatableType<ADSBVehicleManagerSettings>("QGroundControl.SettingsManager", 1, 0,
                                                                 "ADSBVehicleManagerSettings", kRefOnly);

    /* Expose specific singletons */
    (void)qmlRegisterUncreatableType<NTRIPSettings>("QGroundControl", 1, 0, "NTRIPSettings", kRefOnly);
}
    ...
    if (!qgcApp()->runningUnitTests()) {
        ...
        if (groupOverrides.contains(settingsGroup) && groupOverrides[settingsGroup].contains(metaData.name())) {
            ...
            QScopedPointer<FactMetaData> overrideMetaData(
                FactMetaData::createFromJsonObject(settingOverrideJsonObject, {}, nullptr));
