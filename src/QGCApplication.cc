...
static QObject* screenToolsControllerSingletonFactory(QQmlEngine*, QJSEngine*) { return new ScreenToolsController; }
...
void QGCApplication::_initForNormalAppBoot() {
    ...
    // Register QGroundControl palette types for QML
    qmlRegisterType<QGCPalette>("QGroundControl.Palette", 1, 0, "QGCPalette");
    qmlRegisterType<QGCMapPalette>("QGroundControl.Palette", 1, 0, "QGCMapPalette");
    // Register ScreenToolsController singleton for QML (aligned with SkyCleanApp)
    qmlRegisterSingletonType<ScreenToolsController>("QGroundControl.ScreenToolsController", 1, 0,
                                                    "ScreenToolsController", screenToolsControllerSingletonFactory);
    // Ensure Controllers and Airspace modules are present to satisfy QML imports.
    // Registering a generic uncreatable QObject prevents the "module is not installed" error
    // while the concrete types are provided elsewhere in the codebase.
    qmlRegisterUncreatableType<QObject>("QGroundControl.Controllers", 1, 0, "Controllers", "Reference only");
    qmlRegisterUncreatableType<QObject>("QGroundControl.Airspace", 1, 0, "Airspace", "Reference only");
...
void QGCApplication::_checkForNewVersion() {
    ...
    if (!versionCheckFile.isEmpty()) {
        ...
        (void)connect(download, &QGCFileDownload::downloadComplete, this,
                      &QGCApplication::_qgcCurrentStableVersionDownloadComplete);
        download->download(versionCheckFile);
    }
}

void QGCApplication::_qgcCurrentStableVersionDownloadComplete(const QString& remoteFile, const QString& localFile,
...
QString QGCApplication::numberToString(quint64 number) { return getCurrentLanguage().toString(number); }
