...
GPSProvider::GPSProvider(const QString& device, GPSType type, const rtk_data_s& rtkData,
                         const std::atomic_bool& requestStop, QObject* parent)
    : QThread(parent), _device(device), _type(type), _requestStop(requestStop), _rtkData(rtkData) {
    // qCDebug(GPSProviderLog) << Q_FUNC_INFO << this;

    qCDebug(GPSProviderLog) << QString("Survey in accuracy: %1 | duration: %2")
                                   .arg(_rtkData.surveyInAccMeters)
                                   .arg(_rtkData.surveyInDurationSecs);
}
...
void GPSProvider::_publishSatelliteInfo() { emit satelliteInfoUpdate(_satelliteInfo); }

void GPSProvider::_publishSensorGNSSRelative() { emit sensorGnssRelativeUpdate(_sensorGnssRelative); }

void GPSProvider::_publishSensorGPS() { emit sensorGpsUpdate(_sensorGps); }
...
GPSBaseStationSupport* GPSProvider::_connectGPS() {
    ...
    switch (_type) {
        ...
        case GPSType::u_blox: {
            ...
            settings.dynamic_model = 0;
            settings.dgnss_timeout = 0;
            settings.min_cno = 0;
            settings.min_elev = 0;
            settings.heading_offset = kGPSHeadingOffset;
            settings.uart2_baudrate = 0;
            settings.ppk_output = false;
            settings.mode = GPSDriverUBX::UBXMode::Normal;

            gpsDriver = new GPSDriverUBX(GPSDriverUBX::Interface::UART, &_callbackEntry, this, &_sensorGps,
    ...
    switch (_rtkData.useFixedBaseLocation) {
        case BaseModeDefinition::Mode::BaseFixed:
            gpsDriver->setBasePosition(_rtkData.fixedBaseLatitude, _rtkData.fixedBaseLongitude,
                                       _rtkData.fixedBaseAltitudeMeters, _rtkData.fixedBaseAccuracyMeters * 1000.0f);
            break;
