...
StatusText::StatusText(MAV_COMPONENT componentid, MAV_SEVERITY severity, const QString& text)
    : m_compId(componentid), m_severity(severity), m_text(text) {
...
void StatusTextHandler::handleHTMLEscapedTextMessage(MAV_COMPONENT compId, MAV_SEVERITY severity, const QString& text,
    ...
    const QString formatText = QString("<font style=\"%1\">[%2 %3] %4: %5</font><br/>")
                                   .arg(style, dateString, compString, severityText, htmlText);
...
void StatusTextHandler::_handleStatusText(const mavlink_message_t& message) {
    ...
    if (m_chunkedStatusTextInfoMap.contains(compId) &&
        (m_chunkedStatusTextInfoMap.value(compId).chunkId != statustext.id)) {
