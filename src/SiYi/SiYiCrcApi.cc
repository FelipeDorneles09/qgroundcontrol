...
const uint32_t crc32_tab[256] = {
    ...
    0x128e9dcf, 0x164f8078, 0x1b0ca6a1, 0x1fcdbb16, 0x18aeb13,  0x54bf6a4,  0x808d07d,  0xcc9cdca,  0x7897ab07,
    ...
    0x251d3b9e, 0x21dc2629, 0x2c9f00f0, 0x285e1d47, 0x36194d42, 0x32d850f5, 0x3f9b762c, 0x3b5a6b9b, 0x315d626,
    0x7d4cb91,  0xa97ed48,  0xe56f0ff,  0x1011a0fa, 0x14d0bd4d, 0x19939b94, 0x1d528623, 0xf12f560e, 0xf5ee4bb9,
    ...
    0x65c52d24, 0x119b4be9, 0x155a565e, 0x18197087, 0x1cd86d30, 0x29f3d35,  0x65e2082,  0xb1d065b,  0xfdc1bec,
...
static uint32_t CRC32_cal(uint8_t* ptr, uint32_t len, uint16_t crc_init) {
    uint32_t crc = crc_init;
    uint8_t temp;

    while (len-- != 0) {
        temp = (crc >> 24) & 0xff;
        uint32_t oldcrc32 = crc32_tab[*ptr ^ temp];
        crc = (crc << 8) ^ oldcrc32;
        ptr++;
    }
    return crc;
...
SiYiCrcApi::SiYiCrcApi(QObject* parent) : QObject(parent) {}

quint32 SiYiCrcApi::calculateCrc32(const QByteArray& bytes) {
    QByteArray input = bytes;
    uint32_t ret = CRC32_cal(reinterpret_cast<uint8_t*>(input.data()), input.length(), 0);
    return ret;
}
