...
MissionController::MissionController(PlanMasterController* masterController, QObject* parent)
    ...
    connect(_planViewSettings->takeoffItemNotRequired(), &Fact::rawValueChanged, this,
            &MissionController::_forceRecalcOfAllowedBits);
    connect(_planViewSettings->allowMultipleLandingPatterns(), &Fact::rawValueChanged, this,
            &MissionController::multipleLandPatternsAllowedChanged);
    connect(_masterController, &PlanMasterController::managerVehicleChanged, this,
            &MissionController::multipleLandPatternsAllowedChanged);
    connect(this, &MissionController::multipleLandPatternsAllowedChanged, this,
            &MissionController::_forceRecalcOfAllowedBits);
...
void MissionController::_resetMissionFlightStatus(void) {
    ...
    if (_missionFlightStatus.mAhBattery != 0) {
        double batteryPercentRemainingAnnounce =
            SettingsManager::instance()->appSettings()->batteryPercentRemainingAnnounce()->rawValue().toDouble();
        _missionFlightStatus.ampMinutesAvailable = static_cast<double>(_missionFlightStatus.mAhBattery) / 1000.0 *
                                                   60.0 * ((100.0 - batteryPercentRemainingAnnounce) / 100.0);
    }
...
void MissionController::_newMissionItemsAvailableFromVehicle(bool removeAllRequested) {
    ...
    if (_flyView || removeAllRequested || _itemsRequested || isEmpty()) {
        ...
        // We set Altitude mode to mixed, otherwise if we need a non relative altitude frame we won't be able to change
        // it
...
VisualMissionItem* MissionController::_insertSimpleMissionItemWorker(QGeoCoordinate coordinate, MAV_CMD command,
...
VisualMissionItem* MissionController::insertSimpleMissionItem(QGeoCoordinate coordinate, int visualItemIndex,
...
VisualMissionItem* MissionController::insertTakeoffItem(QGeoCoordinate /*coordinate*/, int visualItemIndex,
                                                        bool makeCurrentItem) {
    int sequenceNumber = _nextSequenceNumber();
    _takeoffMissionItem =
...
bool MissionController::multipleLandPatternsAllowed(void) const {
    ...
    return _planViewSettings->allowMultipleLandingPatterns()->rawValue().toBool() &&
           !_masterController->managerVehicle()->px4Firmware();
}

VisualMissionItem* MissionController::insertLandItem(QGeoCoordinate coordinate, int visualItemIndex,
                                                     bool makeCurrentItem) {
    if (_controllerVehicle->fixedWing()) {
        ...
        return _insertSimpleMissionItemWorker(
            coordinate, _controllerVehicle->vtol() ? MAV_CMD_NAV_VTOL_LAND : MAV_CMD_NAV_RETURN_TO_LAUNCH,
            visualItemIndex, makeCurrentItem);
    }
}

VisualMissionItem* MissionController::insertROIMissionItem(QGeoCoordinate coordinate, int visualItemIndex,
                                                           bool makeCurrentItem) {
...
VisualMissionItem* MissionController::insertComplexMissionItem(QString itemName, QGeoCoordinate mapCenterCoordinate,
                                                               int visualItemIndex, bool makeCurrentItem) {
...
VisualMissionItem* MissionController::insertComplexMissionItemFromKMLOrSHP(QString itemName, QString file,
                                                                           int visualItemIndex, bool makeCurrentItem) {
    ComplexMissionItem* newItem = nullptr;
...
void MissionController::_insertComplexMissionItemWorker(const QGeoCoordinate& mapCenterCoordinate,
                                                        ComplexMissionItem* complexItem, int visualItemIndex,
                                                        bool makeCurrentItem) {
    int sequenceNumber = _nextSequenceNumber();
    ...
    if (surveyStyleItem) {
        ...
        if (_controllerVehicle->firmwarePlugin()->hasGimbal(_controllerVehicle, rollSupported, pitchSupported,
                                                            yawSupported) &&
            pitchSupported) {
...
bool MissionController::_loadJsonMissionFileV1(const QJsonObject& json, QmlObjectListModel* visualItems,
                                               QString& errorString) {
...
bool MissionController::_loadJsonMissionFileV2(const QJsonObject& json, QmlObjectListModel* visualItems,
                                               QString& errorString) {
    // Validate root object keys
    QList<JsonHelper::KeyValidateInfo> rootKeyInfoList = {
    ...
    if (!JsonHelper::loadGeoCoordinate(json[_jsonPlannedHomePositionKey], true /* altitudeRequired */, homeCoordinate,
                                       errorString)) {
        return false;
    }
        ...
        if (itemType == VisualMissionItem::jsonTypeSimpleItemValue) {
            ...
            if (complexItemType == SurveyComplexItem::jsonComplexItemTypeValue) {
                ...
                SurveyComplexItem* surveyItem =
                    new SurveyComplexItem(_masterController, _flyView, QString() /* kmlOrShpFile */);
                ...
                StructureScanComplexItem* structureItem =
                    new StructureScanComplexItem(_masterController, _flyView, QString() /* kmlOrShpFile */);
                ...
                CorridorScanComplexItem* corridorItem =
                    new CorridorScanComplexItem(_masterController, _flyView, QString() /* kmlOrShpFile */);
...
bool MissionController::_loadItemsFromJson(const QJsonObject& json, QmlObjectListModel* visualItems,
                                           QString& errorString) {
...
bool MissionController::_loadTextMissionFile(QTextStream& stream, QmlObjectListModel* visualItems,
                                             QString& errorString) {
    bool firstItem = true;
    ...
    if (versionOk) {
        ...
        errorString =
            tr("The mission file is not compatible with this version of %1.").arg(QCoreApplication::applicationName());
        return false;
    }
...
FlightPathSegment* MissionController::_createFlightPathSegmentWorker(VisualItemPair& pair, bool mavlinkTerrainFrame) {
    ...
    FlightPathSegment* segment = new FlightPathSegment(segmentType, coord1, coord1AMSLAlt, coord2, coord2AMSLAlt,
...
FlightPathSegment* MissionController::_addFlightPathSegment(FlightPathSegmentHashTable& prevItemPairHashTable,
                                                            VisualItemPair& pair, bool mavlinkTerrainFrame) {
    FlightPathSegment* segment = nullptr;

    if (prevItemPairHashTable.contains(pair) && (prevItemPairHashTable[pair]->segmentType() ==
                                                 FlightPathSegment::SegmentTypeTerrainFrame) != mavlinkTerrainFrame) {
        ...
        if (simpleItem) {
            ...
            switch (command) {
                ...
                case MAV_CMD_NAV_VTOL_TAKEOFF:
                    ...
                    if (!linkEndToHome) {
                        // If we still haven't found the first coordinate item and we hit a takeoff command this means
                        // the mission starts from the ground. Link the first item back to home to show that.
...
void MissionController::_addTimeDistance(bool vtolInHover, double hoverTime, double cruiseTime, double extraTime,
...
void MissionController::_deinitVisualItem(VisualMissionItem* visualItem) {
    if (visualItem) {
        // Disconnect signals from this visual item to this MissionController only
...
bool MissionController::_findPreviousAltitude(int newIndex, double* prevAltitude,
                                              QGroundControlQmlGlobal::AltMode* prevAltitudeMode) {
    bool found = false;
...
void MissionController::_scanForAdditionalSettings(QmlObjectListModel* visualItems,
...
void MissionController::setCurrentPlanViewSeqNum(int sequenceNumber, bool force) {
    if (_visualItems && (force || sequenceNumber != _currentPlanViewSeqNum)) {
        ...
        if (_masterController->controllerVehicle()->takeoffVehicleSupported() &&
            !_planViewSettings->takeoffItemNotRequired()->rawValue().toBool() && noItemsAddedYet) {
        ...
        for (int viIndex = 0; viIndex < _visualItems->count(); viIndex++) {
            ...
            if (currentSeqNumber == sequenceNumber) {
                ...
                // Child items will always be later in the mission sequence, so this should be correct, to be sure the
                // HasCurrentChildItem is cleared
                pVI->setHasCurrentChildItem(false);
            }
        }
...
void MissionController::_updateTimeout() {
    ...
    //-- Build bounding "cube"
    boundingCube = QGCGeoBoundingCube(QGeoCoordinate(north - 90.0, west - 180.0, minAlt),
