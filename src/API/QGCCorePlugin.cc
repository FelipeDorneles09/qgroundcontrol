...
// Compatibility with SkyCleanApp: expose whether guided actions controller logging is enabled
bool QGCCorePlugin::guidedActionsControllerLogging() const {
    // Default: logging category not present/enabled in this build, so return false.
    return false;
}
