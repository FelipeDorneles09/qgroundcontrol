...
Item {
    ...
    function confirmAction(actionCode, actionData, mapIndicator) {
        ...
        // If confirmDialog is not available (timing or missing UI), fall back to executing
        // the action immediately with sensible defaults. This matches SkyCleanApp behavior
        // where confirmation UI may present but should not crash the app when missing.
        if (!confirmDialog) {
            console.warn("confirmDialog undefined - executing action immediately")
            // Default altitude change and optionChecked values
