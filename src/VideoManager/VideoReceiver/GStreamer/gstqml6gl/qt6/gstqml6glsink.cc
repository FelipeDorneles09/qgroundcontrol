...
G_DEFINE_TYPE_WITH_CODE(GstQml6GLSink, gst_qml6_gl_sink, GST_TYPE_VIDEO_SINK,
                        GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "qtsink", 0, "Qt Video Sink");
                        G_IMPLEMENT_INTERFACE(GST_TYPE_NAVIGATION, gst_qml6_gl_sink_navigation_interface_init));
...
static void gst_qml6_gl_sink_class_init(GstQml6GLSinkClass* klass) {
    ...
    gst_element_class_set_metadata(gstelement_class, "Qt6 Video Sink", "Sink/Video",
                                   "A video sink that renders to a QQuickItem for Qt6",
                                   "Matthew Waters <matthew@centricular.com>");

    g_object_class_install_property(
        gobject_class, PROP_WIDGET,
        g_param_spec_pointer("widget", "QQuickItem", "The QQuickItem to place in the object hierarchy",
    ...
    g_object_class_install_property(
        ...
        gst_param_spec_fraction("pixel-aspect-ratio", "Pixel Aspect Ratio", "The pixel aspect ratio of the device",
                                DEFAULT_PAR_N, DEFAULT_PAR_D, G_MAXINT, 1, 1, 1,
                                (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
...
static void gst_qml6_gl_sink_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
    ...
    switch (prop_id) {
        case PROP_WIDGET: {
            ...
            if (qt_item) {
                ...
                if (qt_sink->widget) {
                    // ==== THREAD-SAFETY FIX ====
                    // The widget is a Qt object that must be accessed from the Qt render thread.
                    // We use QMetaObject::invokeMethod to safely call setSink from the correct thread.
                    auto widget = qt_sink->widget;
                    auto sink = GST_ELEMENT_CAST(qt_sink);

                    if (widget && widget->videoItem()) {
                        // Use BlockingQueuedConnection to ensure the operation completes
                        // This allows us to safely set up the sink before the pipeline continues
                        QMetaObject::invokeMethod(
                            widget.data(), [widget, sink]() { widget->setSink(sink); }, Qt::BlockingQueuedConnection);
                    }
                }
            } else {
        ...
        case PROP_FORCE_ASPECT_RATIO:
            ...
            // Use BlockingQueuedConnection to ensure the property is set
            if (qt_sink->widget && qt_sink->widget->videoItem()) {
                gboolean value_bool = g_value_get_boolean(value);
                QMetaObject::invokeMethod(
                    qt_sink->widget.data(),
                    [widget = qt_sink->widget, value_bool]() { widget->setForceAspectRatio(value_bool); },
                    Qt::BlockingQueuedConnection);
            }
            break;
        case PROP_PIXEL_ASPECT_RATIO:
            g_return_if_fail(qt_sink->widget);
            // Use BlockingQueuedConnection for thread-safe property setting
            if (qt_sink->widget && qt_sink->widget->videoItem()) {
                gint num = gst_value_get_fraction_numerator(value);
                gint den = gst_value_get_fraction_denominator(value);
                QMetaObject::invokeMethod(
                    qt_sink->widget.data(), [widget = qt_sink->widget, num, den]() { widget->setDAR(num, den); },
                    Qt::BlockingQueuedConnection);
            }
            break;
        default:
...
static void gst_qml6_gl_sink_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
    GstQml6GLSink* qt_sink = GST_QML6_GL_SINK(object);
...
static gboolean gst_qml6_gl_sink_query(GstBaseSink* bsink, GstQuery* query) {
    ...
    switch (GST_QUERY_TYPE(query)) {
        case GST_QUERY_CONTEXT: {
            if (gst_gl_handle_context_query((GstElement*)qt_sink, query, qt_sink->display, qt_sink->context,
                                            qt_sink->qt_context))
                return TRUE;
...
static gboolean gst_qml6_gl_sink_stop(GstBaseSink* bsink) { return TRUE; }
...
static GstStateChangeReturn gst_qml6_gl_sink_change_state(GstElement* element, GstStateChange transition) {
    ...
    GST_DEBUG("changing state: %s => %s", gst_element_state_get_name(GST_STATE_TRANSITION_CURRENT(transition)),
    ...
    switch (transition) {
        case GST_STATE_CHANGE_NULL_TO_READY:
            ...
            if (!app) {
                GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, ("%s", "Failed to connect to Qt"),
                                  ("%s", "Could not retrieve QGuiApplication instance"));
                return GST_STATE_CHANGE_FAILURE;
            }

            if (!qt_sink->widget) {
                GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, ("%s", "Required property \'widget\' not set"), (NULL));
                return GST_STATE_CHANGE_FAILURE;
            }

            if (!qt_sink->widget->initWinSys()) {
                GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, ("%s", "Could not initialize window system"), (NULL));
                return GST_STATE_CHANGE_FAILURE;
            }
            ...
            if (!qt_sink->display || !qt_sink->context || !qt_sink->qt_context) {
                GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND,
                                  ("%s", "Could not retrieve window system OpenGL configuration"), (NULL));
                return GST_STATE_CHANGE_FAILURE;
            }
    ...
    switch (transition) {
        ...
        case GST_STATE_CHANGE_PAUSED_TO_READY:
            if (qt_sink->widget) qt_sink->widget->setBuffer(NULL);
            break;
        ...
        if (GST_BUFFER_DURATION_IS_VALID(buf))
            *end = *start + GST_BUFFER_DURATION(buf);
            ...
            if (GST_VIDEO_INFO_FPS_N(&qt_sink->v_info) > 0) {
                *end = *start + gst_util_uint64_scale_int(GST_SECOND, GST_VIDEO_INFO_FPS_D(&qt_sink->v_info),
...
gboolean gst_qml6_gl_sink_set_caps(GstBaseSink* bsink, GstCaps* caps) {
    GstQml6GLSink* qt_sink = GST_QML6_GL_SINK(bsink);

    GST_DEBUG("set caps with %" GST_PTR_FORMAT, caps);

    if (!gst_video_info_from_caps(&qt_sink->v_info, caps)) return FALSE;

    if (!qt_sink->widget) return FALSE;

    return qt_sink->widget->setCaps(caps);
}

static GstFlowReturn gst_qml6_gl_sink_show_frame(GstVideoSink* vsink, GstBuffer* buf) {
    GstQml6GLSink* qt_sink = GST_QML6_GL_SINK(vsink);

    GST_TRACE("rendering buffer:%p", buf);

    if (qt_sink->widget) qt_sink->widget->setBuffer(buf);
...
static gboolean gst_qml6_gl_sink_propose_allocation(GstBaseSink* bsink, GstQuery* query) {
    ...
    if (!qt_sink->display || !qt_sink->context) return FALSE;

    gst_query_parse_allocation(query, &caps, &need_pool);

    if (caps == NULL) goto no_caps;

    /* FIXME re-using buffer pool breaks renegotiation */
    if ((pool = qt_sink->pool)) gst_object_ref(pool);
    ...
    if (pool != NULL) {
        ...
        if (!gst_video_info_from_caps(&info, caps)) goto invalid_caps;
    ...
    if (qt_sink->context->gl_vtable->FenceSync) gst_query_add_allocation_meta(query, GST_GL_SYNC_META_API_TYPE, 0);
