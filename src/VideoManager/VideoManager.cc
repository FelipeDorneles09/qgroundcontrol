...
VideoManager::VideoManager(QObject* parent)
    ...
    /* Expose VideoManager/VideoReceiver to QML as reference-only (used extensively in QML) */
    (void)qmlRegisterUncreatableType<VideoManager>("QGroundControl.VideoManager", 1, 0, "VideoManager",
                                                   "Reference only");
    (void)qmlRegisterUncreatableType<VideoReceiver>("QGroundControl.VideoManager", 1, 0, "VideoReceiver",
                                                    "Reference only");
#endif
}
...
void FinishVideoInitialization::run() {
    ...
    // Run the initialization on the main thread to avoid creating QObjects
    // in the QSGRenderThread which would cause QObject parent/ thread violations.
    QMetaObject::invokeMethod(
        VideoManager::instance(), []() { VideoManager::instance()->_initAfterQmlIsReady(); }, Qt::QueuedConnection);
}
