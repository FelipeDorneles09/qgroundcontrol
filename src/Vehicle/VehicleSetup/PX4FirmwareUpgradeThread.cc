    ...
    if (_findBoardFromPorts(portInfo, boardType, boardName)) {
        if (!_foundBoard) {
            ...
            if (!_findBoardFirstAttempt) {
                ...
                connect(_bootloader, &Bootloader::updateProgress, this,
                        &PX4FirmwareUpgradeThreadWorker::_updateProgress);
...
bool PX4FirmwareUpgradeThreadWorker::_findBoardFromPorts(QGCSerialPortInfo& portInfo,
                                                         QGCSerialPortInfo::BoardType_t& boardType,
                                                         QString& boardName) {
...
PX4FirmwareUpgradeThreadController::PX4FirmwareUpgradeThreadController(QObject* parent) : QObject(parent) {
    ...
    connect(_worker, &PX4FirmwareUpgradeThreadWorker::foundBoard, this,
            &PX4FirmwareUpgradeThreadController::_foundBoard);
...
PX4FirmwareUpgradeThreadController::~PX4FirmwareUpgradeThreadController() {
    _workerThread->quit();
    _workerThread->wait();
    // _workerThread is parented to this object (created with new(..., this)).
    // It will be deleted automatically by QObject. Avoid manual delete which
    // can cause double-free if parent deletion also occurs.
