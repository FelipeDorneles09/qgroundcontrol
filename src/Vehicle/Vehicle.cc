...
Vehicle::Vehicle(LinkInterface* link, int vehicleId, int defaultComponentId, MAV_AUTOPILOT firmwareType,
    ...
    connect(JoystickManager::instance(), &JoystickManager::activeJoystickChanged, this,
            &Vehicle::_loadJoystickSettings);
    connect(MultiVehicleManager::instance(), &MultiVehicleManager::activeVehicleChanged, this,
            &Vehicle::_activeVehicleChanged);

    qCDebug(VehicleLog) << "Link started with Mavlink "
                        << (MAVLinkProtocol::instance()->getCurrentVersion() >= 200 ? "V2" : "V1");
...
void Vehicle::_commonInit(LinkInterface* link) {
    ...
    // Initialize alt above terrain to Nan so frontend can display it correctly in case the terrain query had no
    // response
...
QString Vehicle::firmwareTypeString() const { return QGCMAVLink::firmwareClassToString(_firmwareType); }
...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message) {
    ...
    if (!_heardFrom) {
        ...
        if (_compID == message.compid) {
            ...
            if (packet_lost_count) emit messagesLostChanged();
        }
    }
...
void Vehicle::_handleEvent(uint8_t comp_id, std::unique_ptr<events::parser::ParsedEvent> event) {
    ...
    if (event->group() == "default" && severity != -1) {
        ...
        if (!message.empty()) {
            ...
            m_statusTextHandler->handleHTMLEscapedTextMessage(static_cast<MAV_COMPONENT>(comp_id),
                                                              static_cast<MAV_SEVERITY>(severity), text,
    ...
    if (eventData == _events.end()) {
        ...
        QSharedPointer<EventHandler> eventHandler{
            new EventHandler(this, profile, std::bind(&Vehicle::_handleEvent, this, compid, std::placeholders::_1),
                             sendRequestEventMessageCB, MAVLinkProtocol::instance()->getSystemId(),
                             MAVLinkProtocol::getComponentId(), _id, compid)};
        ...
        connect(eventHandler.data(), &EventHandler::healthAndArmingChecksUpdated, this, [compid, this]() {
            ...
            _healthAndArmingCheckReport.update(
                compid, eventHandler->healthAndArmingCheckResults(),
                eventHandler->getModeGroup(_has_custom_mode_user_intention ? _custom_mode_user_intention
                                                                           : _custom_mode));
        });
...
void Vehicle::_handleRCChannels(mavlink_message_t& message) {
    ...
    uint16_t* _rgChannelvalues[QGCMAVLink::maxRcChannels] = {
        &channels.chan1_raw,  &channels.chan2_raw,  &channels.chan3_raw,  &channels.chan4_raw,  &channels.chan5_raw,
        &channels.chan6_raw,  &channels.chan7_raw,  &channels.chan8_raw,  &channels.chan9_raw,  &channels.chan10_raw,
        &channels.chan11_raw, &channels.chan12_raw, &channels.chan13_raw, &channels.chan14_raw, &channels.chan15_raw,
        &channels.chan16_raw, &channels.chan17_raw, &channels.chan18_raw,
    };
...
void Vehicle::_loadJoystickSettings() {
    ...
    if (JoystickManager::instance()->activeJoystick()) {
        qCDebug(JoystickLog) << "Vehicle " << this->id()
                             << " Notified of an active joystick. Loading setting joystickenabled: "
                             << settings.value(_joystickEnabledSettingsKey, false).toBool();
...
void Vehicle::virtualTabletJoystickValue(double roll, double pitch, double yaw, double thrust) {
    ...
    if (!_joystickEnabled) {
        sendJoystickDataThreadSafe(static_cast<float>(roll), static_cast<float>(pitch), static_cast<float>(yaw),
                                   static_cast<float>(thrust), 0, 0, NAN, NAN);
    }
}

void Vehicle::_say(const QString& text) { AudioOutput::instance()->say(text.toLower()); }
...
bool Vehicle::spacecraft() const { return QGCMAVLink::isSpacecraft(vehicleType()); }
...
QString Vehicle::vehicleTypeString() const { return QGCMAVLink::mavTypeToString(_vehicleType); }

QString Vehicle::vehicleClassInternalName() const { return QGCMAVLink::vehicleClassToInternalString(vehicleClass()); }
...
double Vehicle::minimumTakeoffAltitudeMeters() { return _firmwarePlugin->minimumTakeoffAltitudeMeters(this); }
...
bool Vehicle::hasGripper() const { return _firmwarePlugin->hasGripper(this); }

void Vehicle::startTakeoff() { _firmwarePlugin->startTakeoff(this); }
...
void Vehicle::guidedModeROI(const QGeoCoordinate& centerCoord) {
    ...
    if (capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        sendMavCommandInt(defaultComponentId(), MAV_CMD_DO_SET_ROI_LOCATION,
...
bool Vehicle::inFwdFlight() const { return fixedWing() || _vtolInFwdFlight; }
...
void Vehicle::sendMavCommandDelayed(int compId, MAV_CMD command, bool showError, int milliseconds, float param1,
    ...
    QTimer::singleShot(milliseconds, this, [=, this] {
        sendMavCommand(compId, command, showError, param1, param2, param3, param4, param5, param6, param7);
    });
}
...
void Vehicle::sendMavCommandWithHandler(const MavCmdAckHandlerInfo_t* ackHandlerInfo, int compId, MAV_CMD command,
    ...
    _sendMavCommandWorker(false,  // commandInt
                          false,  // showError
                          ackHandlerInfo, compId, command, MAV_FRAME_GLOBAL, param1, param2, param3, param4, param5,
                          param6, param7);
}
...
void Vehicle::sendMavCommandIntWithHandler(const MavCmdAckHandlerInfo_t* ackHandlerInfo, int compId, MAV_CMD command,
    ...
    _sendMavCommandWorker(true,   // commandInt
                          false,  // showError
                          ackHandlerInfo, compId, command, frame, param1, param2, param3, param4, param5, param6,
                          param7);
}
...
static void _sendMavCommandWithLambdaFallbackHandler(void* resultHandlerData, int /*compId*/,
...
void Vehicle::sendMavCommandWithLambdaFallback(std::function<void()> lambda, int compId, MAV_CMD command,
    ...
    switch (instanceData->getCommandSupported(command)) {
        ...
        case FirmwarePluginInstanceData::CommandSupportedResult::UNKNOWN: {
            ...
            const MavCmdAckHandlerInfo_t handlerInfo{/* .resultHandler = */ &_sendMavCommandWithLambdaFallbackHandler,
                                                     /* .resultHandlerData =  */ data,
                                                     /* .progressHandler =  */ nullptr,
                                                     /* .progressHandlerData =  */ nullptr};
            sendMavCommandWithHandler(&handlerInfo, compId, command, param1, param2, param3, param4, param5, param6,
                                      param7);
            break;
        }
    }
...
bool Vehicle::isMavCommandPending(int targetCompId, MAV_CMD command) {
    ...
    // qDebug() << "Pending target: " << targetCompId << ", command: " << (int)command << ", pending: " << (pending ?
    // "yes" : "no");
    return pending;
}
...
bool Vehicle::_sendMavCommandShouldRetry(MAV_CMD command) {
    switch (command) {
        ...
        case MockLink::MAV_CMD_MOCKLINK_NO_RESPONSE:
            ...
            // In general we should not retry any commands. This is for safety reasons. For example you don't want an
            // ARM command to timeout with no response over a noisy link twice and then suddenly have the third try work
            // 6 seconds later. At that point the user could have walked up to the vehicle to see what is going wrong.
            //
            // We do retry commands which are part of the initial vehicle connect sequence. This makes this process work
            // better over noisy links where commands could be lost. Also these commands tend to just be requesting
            // status so if they end up being delayed there are no safety concerns that could occur.
...
void Vehicle::_sendMavCommandWorker(bool commandInt, bool showError, const MavCmdAckHandlerInfo_t* ackHandlerInfo,
                                    int targetCompId, MAV_CMD command, MAV_FRAME frame, float param1, float param2,
                                    float param3, float param4, double param5, double param6, float param7) {
    // We can't send commands to compIdAll using this method. The reason being that we would get responses back possibly
    // from multiple components which this code can't handle. We also can't send the majority of commands again if we
    // are already waiting for a response from that same command. If we did that we would not be able to discern which
    // ack was associated with which command.
    if ((targetCompId == MAV_COMP_ID_ALL) ||
        ...
        qCDebug(VehicleLog) << QStringLiteral("_sendMavCommandWorker failing %1")
                                   .arg(compIdAll ? "MAV_COMP_ID_ALL not supported" : "duplicate command")
                            << rawCommandName << param1 << param2 << param3 << param4 << param5 << param6 << param7;
    ...
    qCDebug(VehicleLog) << Q_FUNC_INFO << "command:param1-7" << command << param1 << param2 << param3 << param4
...
void Vehicle::_sendMavCommandFromList(int index) {
    ...
    if (++_mavCommandList[index].tryCount > commandEntry.maxTries) {
        ...
        if (commandEntry.ackHandlerInfo.resultHandler) {
            mavlink_command_ack_t ack = {};
            ack.result = MAV_RESULT_FAILED;
            (*commandEntry.ackHandlerInfo.resultHandler)(commandEntry.ackHandlerInfo.resultHandlerData,
                                                         commandEntry.targetCompId, ack,
    ...
    qCDebug(VehicleLog) << Q_FUNC_INFO << "command:tryCount:param1-7" << rawCommandName << commandEntry.tryCount
                        << commandEntry.rgParam1 << commandEntry.rgParam2 << commandEntry.rgParam3
                        << commandEntry.rgParam4 << commandEntry.rgParam5 << commandEntry.rgParam6
                        << commandEntry.rgParam7;
    ...
    if (entryIndex != -1) {
        if (ack.result == MAV_RESULT_IN_PROGRESS) {
            ...
            if (px4Firmware() && ack.command == MAV_CMD_DO_AUTOTUNE_ENABLE) {
                ...
                commandEntryRef.elapsedTimer
                    .start();  // We've heard from vehicle, restart elapsed timer for no ack received timeout
                commandEntry = commandEntryRef;
            }

            if (commandEntry.ackHandlerInfo.progressHandler) {
                (*commandEntry.ackHandlerInfo.progressHandler)(commandEntry.ackHandlerInfo.progressHandlerData,
                                                               message.compid, ack);
            }
        } else {
            ...
            if (commandEntry.ackHandlerInfo.resultHandler) {
                (*commandEntry.ackHandlerInfo.resultHandler)(commandEntry.ackHandlerInfo.resultHandlerData,
                                                             message.compid, ack, MavCmdResultCommandResultOnly);
            } else {
                if (commandEntry.showError) {
...
void Vehicle::_waitForMavlinkMessageMessageReceivedHandler(const mavlink_message_t& message) {
    if (_requestMessageInfoMap.contains(message.compid) &&
        _requestMessageInfoMap[message.compid].contains(message.msgid)) {
        ...
        for (auto& compIdEntry : _requestMessageInfoMap) {
            for (auto requestMessageInfo : compIdEntry) {
                if (requestMessageInfo->messageWaitElapsedTimer.isValid() &&
                    requestMessageInfo->messageWaitElapsedTimer.elapsed() >
                        (qgcApp()->runningUnitTests() ? 50 : 1000)) {
                    auto resultHandler = requestMessageInfo->resultHandler;
                    auto resultHandlerData = requestMessageInfo->resultHandlerData;

                    qCDebug(VehicleLog) << Q_FUNC_INFO << "request message timed out - compId:msgId"
                                        << requestMessageInfo->compId << requestMessageInfo->msgId;

                    _removeRequestMessageInfo(requestMessageInfo->compId, requestMessageInfo->msgId);

                    mavlink_message_t message;
                    (*resultHandler)(resultHandlerData, MAV_RESULT_FAILED, RequestMessageFailureMessageNotReceived,
                                     message);
...
void Vehicle::_requestMessageCmdResultHandler(void* resultHandlerData_, [[maybe_unused]] int compId,
    ...
    if (requestMessageInfo->messageReceived) {
        // This should never happen. The command should have already been removed from the list when the message was
        // received
        qWarning() << Q_FUNC_INFO
                   << "Command result handler should now have been called if message has already been received";
    } else {
...
void Vehicle::_requestMessageWaitForMessageResultHandler(void* resultHandlerData, bool noResponsefromVehicle,
    ...
    if (pInfo->commandAckReceived) {
        (*pInfo->resultHandler)(
            pInfo->resultHandlerData, noResponsefromVehicle ? MAV_RESULT_FAILED : MAV_RESULT_ACCEPTED,
...
void Vehicle::requestMessage(RequestMessageResultHandler resultHandler, void* resultHandlerData, int compId,
    ...
    _sendMavCommandWorker(false,  // commandInt
                          false,  // showError
                          &handlerInfo, compId, MAV_CMD_REQUEST_MESSAGE, MAV_FRAME_GLOBAL, messageId, param1, param2,
...
QString Vehicle::motorDetectionFlightMode() const { return _firmwarePlugin->motorDetectionFlightMode(); }

QString Vehicle::stabilizedFlightMode() const { return _firmwarePlugin->stabilizedFlightMode(); }
...
int Vehicle::versionCompare(const QString& compare) const { return _firmwarePlugin->versionCompare(this, compare); }
...
void Vehicle::doSetHome(const QGeoCoordinate& coord) {
    if (coord.isValid()) {
        // If for some reason we already did a query and it hasn't arrived yet, disconnect signals and unset current
        // query. TerrainQuery system will automatically delete that forgotten query. This could happen if we send 2
        // do_set_home commands one after another, so usually the latest one is the one we would want to arrive to the
        // vehicle, so it is fine to forget about the previous ones in case it could happen.
        if (_currentDoSetHomeTerrainAtCoordinateQuery) {
            disconnect(_currentDoSetHomeTerrainAtCoordinateQuery, &TerrainAtCoordinateQuery::terrainDataReceived, this,
                       &Vehicle::_doSetHomeTerrainReceived);
            _currentDoSetHomeTerrainAtCoordinateQuery = nullptr;
        }
        // Save the coord for using when our terrain data arrives. If there was a pending terrain query paired with an
        // older coordinate it is safe to Override now, as we just disconnected the signal that would trigger the
        // command sending
        ...
        connect(_currentDoSetHomeTerrainAtCoordinateQuery, &TerrainAtCoordinateQuery::terrainDataReceived, this,
                &Vehicle::_doSetHomeTerrainReceived);
...
void Vehicle::_doSetHomeTerrainReceived(bool success, QList<double> heights) {
    if (success) {
        ...
        if (_doSetHomeCoordinate.isValid() && terrainAltitude <= SET_HOME_TERRAIN_ALT_MAX &&
            terrainAltitude >= SET_HOME_TERRAIN_ALT_MIN) {
            sendMavCommand(defaultComponentId(), MAV_CMD_DO_SET_HOME,
                           true,  // show error if fails
...
void Vehicle::_updateAltAboveTerrain() {
    ...
    // This is not super elegant but it works to limit the amount of queries we do. It seems more than 500ms is not
    // possible to get serviced on time. It is not a big deal if it is not serviced on time as terrain queries can
    // manage that just fine, but QGC would use resources to service those queries, and it is pointless, so this is a
    // quick workaround to not waste that little computing time
    int altitudeAboveTerrQueryMinInterval = 500;
    if (_altitudeAboveTerrQueryTimer.elapsed() < altitudeAboveTerrQueryMinInterval) {
        // qCDebug(VehicleLog) << "_updateAltAboveTerrain: minimum 500ms interval between queries not reached,
        // returning";
        return;
    }
    // Sanity check, although it is very unlikely that vehicle coordinate is not valid
    if (_coordinate.isValid()) {
        // Check for minimum distance and altitude traveled before doing query, to not do a lot of queries of the same
        // data
        if (_altitudeAboveTerrLastCoord.isValid() && !qIsNaN(_altitudeAboveTerrLastRelAlt)) {
            if (_altitudeAboveTerrLastCoord.distanceTo(_coordinate) < minimumDistanceTraveled &&
                fabs(_altitudeRelativeFact.rawValue().toFloat() - _altitudeAboveTerrLastRelAlt) <
                    minimumAltitudeChanged) {
                return;
            }
        }
        ...
        // If for some reason we already did a query and it hasn't arrived yet, disconnect signals and unset current
        // query. TerrainQuery system will automatically delete that forgotten query.
        if (_altitudeAboveTerrTerrainAtCoordinateQuery) {
            ...
            disconnect(_altitudeAboveTerrTerrainAtCoordinateQuery, &TerrainAtCoordinateQuery::terrainDataReceived, this,
                       &Vehicle::_altitudeAboveTerrainReceived);
        ...
        connect(_altitudeAboveTerrTerrainAtCoordinateQuery, &TerrainAtCoordinateQuery::terrainDataReceived, this,
                &Vehicle::_altitudeAboveTerrainReceived);
        QList<QGeoCoordinate> rgCoord;
...
void Vehicle::sendParamMapRC(const QString& paramName, double scale, double centerValue, int tuningID, double minValue,
    ...
    mavlink_msg_param_map_rc_pack_chan(
        ...
        static_cast<uint8_t>(MAVLinkProtocol::getComponentId()), sharedLink->mavlinkChannel(), &message, _id,
...
void Vehicle::clearAllParamMapRC(void) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        mavlink_msg_param_map_rc_pack_chan(static_cast<uint8_t>(MAVLinkProtocol::instance()->getSystemId()),
                                           static_cast<uint8_t>(MAVLinkProtocol::getComponentId()),
                                           sharedLink->mavlinkChannel(), &message, _id, MAV_COMP_ID_AUTOPILOT1,
                                           param_id_cstr,
...
void Vehicle::sendJoystickDataThreadSafe(float roll, float pitch, float yaw, float thrust, quint16 buttons,
                                         quint16 buttons2, float gimbalPitch, float gimbalYaw) {
    SharedLinkInterfacePtr sharedLink = vehicleLinkManager()->primaryLink().lock();
    if (!sharedLink) {
    ...
    mavlink_msg_manual_control_pack_chan(
        ...
        static_cast<int16_t>(newGimbalPitch), static_cast<int16_t>(newGimbalYaw), 0, 0, 0, 0, 0, 0);
    sendMessageOnLinkThreadSafe(sharedLink.get(), message);
}
...
void Vehicle::_requestMessageMessageIntervalResultHandler(void* resultHandlerData, MAV_RESULT result,
...
void Vehicle::_requestMessageInterval(uint8_t compId, uint16_t msgId) {
    if (!_unsupportedMessageIds.contains(compId, msgId)) {
        requestMessage(&Vehicle::_requestMessageMessageIntervalResultHandler, this, compId,
                       MAVLINK_MSG_ID_MESSAGE_INTERVAL, msgId);
    }
}
...
void Vehicle::_setMessageRateCommandResultHandler(void* resultHandlerData, int compId, const mavlink_command_ack_t& ack,
                                                  MavCmdResultFailureCode_t failureCode) {
...
void Vehicle::setMessageRate(uint8_t compId, uint16_t msgId, int32_t rate) {
    const MavCmdAckHandlerInfo_t handlerInfo = {/* .resultHandler = */ &Vehicle::_setMessageRateCommandResultHandler,
    ...
    sendMavCommandWithHandler(&handlerInfo, compId, MAV_CMD_SET_MESSAGE_INTERVAL, msgId, interval);
}
...
void Vehicle::_textMessageReceived(MAV_COMPONENT componentid, MAV_SEVERITY severity, QString text,
                                   QString description) {
...
void Vehicle::sendSetupSigning() {
    ...
    mavlink_message_t msg;
    (void)mavlink_msg_setup_signing_encode_chan(MAVLinkProtocol::instance()->getSystemId(),
                                                MAVLinkProtocol::getComponentId(), channel, &msg, &setup_signing);

    // Since we don't get an ack back that the message was received send twice to try to make sure it makes it to the
    // vehicle
...
void Vehicle::_createImageProtocolManager() {
    ...
    (void)connect(_imageProtocolManager, &ImageProtocolManager::imageReady, this,
                  [this](const QImage& image) { qgcApp()->qgcImageProvider()->setImage(image, _id); });
}
...
void Vehicle::_createMAVLinkLogManager() { _mavlinkLogManager = new MAVLinkLogManager(this, this); }

MAVLinkLogManager* Vehicle::mavlinkLogManager() const { return _mavlinkLogManager; }
