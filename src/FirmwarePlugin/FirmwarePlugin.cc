...
FirmwarePlugin::FirmwarePlugin(QObject* parent) : QObject(parent) { qCDebug(FirmwarePluginLog) << this; }

FirmwarePlugin::~FirmwarePlugin() { qCDebug(FirmwarePluginLog) << this; }
...
void FirmwarePlugin::_getParameterMetaDataVersionInfo(const QString& metaDataFile, int& majorVersion,
...
void FirmwarePlugin::startMission(Vehicle*) const { qgcApp()->showAppMessage(guided_mode_not_supported_by_vehicle); }
...
const QVariantList& FirmwarePlugin::toolIndicators(const Vehicle*) {
    ...
    if (_toolIndicatorList.isEmpty()) {
        _toolIndicatorList = QVariantList({
            QVariant::fromValue(QUrl::fromUserInput("qrc:/qml/QGroundControl/Toolbar/ROIIndicator.qml")),
...
QGCCameraManager* FirmwarePlugin::createCameraManager(Vehicle* vehicle) const { return new QGCCameraManager(vehicle); }

MavlinkCameraControl* FirmwarePlugin::createCameraControl(const mavlink_camera_information_t* info, Vehicle* vehicle,
                                                          int compID, QObject* parent) const {
...
void FirmwarePlugin::checkIfIsLatestStable(Vehicle* vehicle) const {
    ...
    (void)connect(downloader, &QGCFileDownload::downloadComplete, this,
                  [vehicle, this](const QString& remoteFile, const QString& localFile, const QString& errorMsg) {
                      if (errorMsg.isEmpty()) {
...
Autotune* FirmwarePlugin::createAutotune(Vehicle* vehicle) const { return new Autotune(vehicle); }
